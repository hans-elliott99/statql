...
ARRP db_coltypes(sqlite3 *db, size_t ncols) {
    ARRP v = alloc_array(ncols, INTS_ARR);
...
ARRP db_colnames(sqlite3 *db, size_t ncols) {
    ARRP v = alloc_array(ncols, STRINGS_ARR);
    const char *query = "SELECT * FROM birthwt LIMIT 1";
...
int main3() {
    ...
    */
    for (size_t i = 0; i < tab.ncols; i++) {
        printf("%s (%s) \n",
               strings_elt(tab.colnames, i),
               arrtype_str(ints_elt(tab.coltypes, i))
               );
    }
    return 0;
}
